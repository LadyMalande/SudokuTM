...
namespace sudokuTM
{
    public partial class Solver1 : Form
    {
        ...
        public Solver1()
        {
            ...
            for (int i = 1; i < 10; i++)
            {
                for (int j = 1; j < 10; j++)
                {
                    ...
                    GridButton.ListOfGridButtons.Add(SolverGrid[i - 1, j - 1]);
                }
            }
            KeyPreview = true;
            ListOfNotSolvedButtons = new List<GridButton>(GridButton.ListOfGridButtons);
            /* SolverGrid[0, 0].Text = " ";
             SolverGrid[0, 1].Text = " ";
             SolverGrid[0, 2].Text = " ";
             SolverGrid[0, 3].Text = " ";
        ...
        /// <summary>
        /// Doplní do mřížky pouze ty buňky, které měly pouze jedno možné číslo na vyplnění zjištěné pomocí funkce Sudoku.CanBeFilledIn(int, GridButton)
        /// </summary>
        ...
        private void RefreshNumberOfSolutions( List<GridButton> ListOfNotSolvedButtons)
        {
            ...
            foreach (GridButton NotSolvedGridButton in ListOfNotSolvedButtons)
            {
                ...
                for (int i = 1; i < 10; i++)
                {
                    ...
                    if (Sudoku.CanBeFilledIn(i, NotSolvedGridButton))
                    {
        ...
        public bool BruteForceSolving()
        {
            ...
            {
                if(Sudoku.CanBeFilledIn(Number, NotSolvedButton))
                {
                    ...
                    for (int i = 0; i < 9; i++)
                    {
                        for (int j = 0; j < 9; j++)
                        {
                            if (RowNumberCount[i, j] == 1)
                            {
                                ...
                                {
                                    Sudoku.WhereIWantToFillInNumber = NotSolvedGridButton;
                                    if ((NotSolvedGridButton.Row == (i + 1)) && (Sudoku.CanBeFilledIn(j + 1, NotSolvedGridButton)))
                                    {
                            ...
                            else if (ColumnNumberCount[i, j] == 1)
                            {
                                ...
                                {
                                    Sudoku.WhereIWantToFillInNumber = NotSolvedGridButton;
                                    if ((NotSolvedGridButton.Column == i + 1) && (Sudoku.CanBeFilledIn(j + 1, NotSolvedGridButton)))
                                    {
                                        NotSolvedGridButton.Text = (j + 1).ToString();
                                        ListOfAlreadySolvedButtons.Add(NotSolvedGridButton);
                                        Sudoku.FilledInFields++;
